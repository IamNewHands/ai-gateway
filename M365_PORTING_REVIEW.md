...
- **现状**：`canContinue` 是**死代码**（无任何调用）；即便调用也与原版不符——缺 `repeatedFailure` 和 `pending>0` 检查，默认轮数 8（原版 512）；`buildAgentLedger` 缺「同一失败调用 ≥3 次 → StuckLoop」检测。
...
- **建议**：在 durable 工具循环入口接入 ledger 门禁，恢复 512 默认值及 StuckLoop/RepeatedFailure 判定。
...
#### 5. 远程图片附件下载失败被静默跳过
- **位置**：`src/m365/chathub.ts`（`safeFetchImage` / 上传循环）
- **现状**：下载失败、重定向超限、URL 不合法、非 2xx 一律 `null` → `continue` 跳过该附件继续对话；超 10MiB 也直接跳过。
- **原版**：任一附件失败即整个请求失败（`upload attachment: ...`）。
- **影响**：用户以为图片已上传，实际被忽略，对话上下文缺失。
- **建议**：对齐原版——附件失败时整请求报错，或至少向客户端精确传递各附件失败原因（二选一需确认产品预期）。

#### 6. WS dial / 流式路径不读取 Retry-After 与 keepalive
...
#### 7. WS URL 缺 query 参数（`XRoutingParameterSessionKey`、`isEdu`）
- **位置**：`src/m365/chathub.ts`（`buildWSURL`）
- **现状**：URL 缺 `XRoutingParameterSessionKey`（原版 = requestID）与 `isEdu=false`；`licenseType`/`scenario` 硬编码默认值。
- **原版**：`client.go` 带这些参数，`licenseType`/`scenario` 可由请求覆盖。
- **影响**：会话路由 / 租户判定可能有偏差。
- **建议**：在 `buildWSURL` 补上两个 query 参数，并放开后可覆盖的 `licenseType`/`scenario`。

#### 8. 会话前缀匹配缺原子边界检查
- **位置**：`src/m365/session.ts`（`bindSession` / 上下文前缀匹配）
- **现状**：`contextPrefixLen` 只做逐条相等比较。
- **原版**：用 `buildAtoms` 验证历史必须结束在消息原子边界（不能切在 `assistant.tool_calls` + tool 往返中间），否则返回 0。
- **影响**：`historyLen` 可能落在工具往返中间 → 增量发送从 tool 消息中途开始，云端对话状态错乱。
- **建议**：前缀 matched 的 message 计数处增加原子边界校验。

#### 9. 请求画像缩水：`variants` / `allowedMessageTypes` / `optionsSets` / `clientInfo`
...
#### 10. `mapChatError` 把原始上游错误透给客户端
- **位置**：`src/m365/durable.ts`（`mapChatError`）
- **现状**：兜底返回 500 且把原始上游错误消息透出；content_policy 返回 502。
- **原版**：一律 502，不泄漏内部细节（content_policy → 503）。
- **影响**：内部细节/上游原文可能暴露给端侧；状态码语义偏差。
- **建议**：兜底改 502/503，剥离原始消息。

#### 11. 热账号选择策略差异（LRU vs 粘性+RR）
- **位置**：`src/m365/oauth.ts`（账号池排序）、`src/m365/durable.ts`（failover）
- **现状**：账号选择用 LRU（`lastUsedAt` 升序）；failover 遍历全部候选；全部冷却返回 401 `auth_error`。
- **原版**：粘住上次健康账号 + round-robin（最多探测 16 个）；全冷却 → 429 + Retry-After=最早恢复；failover 只重试一个 next 账号。
- **影响**：账号切换行为与恢复提示语义不同，可能轻抖账号/返回错误码不准确。
- **建议**：按需对齐粘性选择与全冷却 429 语义。

#### 12. `toolProtocolPrompt` 分支方向相反 + 缺防截断前缀
...
#### 13. `clientPlugins` 缺 mcp-gateway 插件条目
...
#### 14. 工具结果截断算法与原版不同（上下文损失 ~40%）
- **位置**：`src/m365/tools.ts`（`compactToolResult`）
- **现状**：head=`max*0.6-40`、tail=80，4000 上限时只保留约 2440 字符；无 TrimSpace、无 200 下限；截断计数口径不同。
- **原版**：head=`limit/3`≈1333、tail=`limit-head-80`≈2587，4000 上限保留约 3920 字符；先 TrimSpace、有下限。
- **影响**：工具结果注入 prompt/ledger 的上下文量比原版少约 40%。
- **建议**：对齐截断算法。

#### 15. 提示词管道差异（`flattenPromptMessages`、`isToolRefusal` / `isSandboxHallucination`）
...
#### 16. `validateDetectedToolCalls` 缺 tool_choice 校验 + 空 arguments 默认
...
#### 19. 图片生成忽略 `accountId`/`user`，多账号时 token 与生成账号错配
...
#### 21. 换 token 端点 no-op 三元 + ROPC 端点硬编码
...
#### 23. Designer 下载重定向语义与 Content-Type 校验差异
...
以下 32 项差异已在 `904a79f`、`b1433cb`、`6f43ac7`（历次修复）中解决或主动调整，本文不再展开：
